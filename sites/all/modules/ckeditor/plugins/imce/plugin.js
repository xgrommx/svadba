...
( function () {
  CKEDITOR.plugins.add('imce', {
      init:function (editor) {
        ...
        editor.addCommand('IMCEWindow', {
          exec:function () {
            var width = editor.config.filebrowserWindowWidth || '80%', height = editor.config.filebrowserWindowHeight || '70%';

            editor.popup(Drupal.settings.basePath + 'index.php?q=imce\x26app=ckeditor|sendto@ckeditor_setFile|&CKEditorFuncNum=' + editor._.filebrowserFnIMCE,
              width, height);
          }
        });
  ...
  function setFile(file) {
    var sel = this.getSelection(), text = sel.getSelectedText();
