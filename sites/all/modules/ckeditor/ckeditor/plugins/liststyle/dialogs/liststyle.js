...
(function () {
  ...
  function d(e, f) {
    ...
    if (f == 'bulletedListStyle') {
      return{title:g.bulletedTitle, minWidth:300, minHeight:50, contents:[
        {id:'info', accessKey:'I', elements:[
          {type:'select', label:g.type, id:'type', align:'center', style:'width:150px', items:[
            [g.notset, ''],
            [g.circle, 'circle'],
            [g.disc, 'disc'],
            [g.square, 'square']
      ...
      if (f == 'numberedListStyle') {
        var h = [
          [g.notset, ''],
          [g.lowerRoman, 'lower-roman'],
          [g.upperRoman, 'upper-roman'],
          [g.lowerAlpha, 'lower-alpha'],
          [g.upperAlpha, 'upper-alpha'],
          [g.decimal, 'decimal']
        ];
        if (!CKEDITOR.env.ie || CKEDITOR.env.version > 7) {
          h.concat([
            [g.armenian, 'armenian'],
            [g.decimalLeadingZero, 'decimal-leading-zero'],
            [g.georgian, 'georgian'],
            [g.lowerGreek, 'lower-greek']
          ]);
        }
        return{title:g.numberedTitle, minWidth:300, minHeight:50, contents:[
          {id:'info', accessKey:'I', elements:[
            {type:'hbox', widths:['25%', '75%'], children:[
              {label:g.start, type:'text', id:'start', validate:CKEDITOR.dialog.validate.integer(g.validateStartNumber), setup:function (i) {
