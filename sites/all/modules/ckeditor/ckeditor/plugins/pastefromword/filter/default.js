...
(function () {
  ...
  ;
  var d = /^([.\d]*)+(em|ex|px|gd|rem|vw|vh|vm|ch|mm|cm|in|pt|pc|deg|rad|ms|s|hz|khz){1}?/i, e = /^(?:\b0[^\s]*\s*){1,4}$/, f = '^m{0,4}(cm|cd|d?c{0,3})(xc|xl|l?x{0,3})(ix|iv|v?i{0,3})$', g = new RegExp(f), h = new RegExp(f.toUpperCase()), i = {decimal:/\d+/, 'lower-roman':g, 'upper-roman':h, 'lower-alpha':/^[a-z]+$/, 'upper-alpha':/^[A-Z]+$/}, j = {disc:/[l\u00B7\u2002]/, circle:/[\u006F\u00D8]/, square:/[\u006E\u25C6]/}, k = {ol:i, ul:j}, l = [
    [1000, 'M'],
    [900, 'CM'],
    [500, 'D'],
    [400, 'CD'],
  ...
  var p = 0, q = null, r, s = CKEDITOR.plugins.pastefromword = {utils:{createListBulletMarker:function (u, v) {
    ...
    if ((w = u.removeAnyChildWithName('cke:listbullet')) && w.length && (w = w[0])) {
      u.name = 'cke:li';
      if (v.style) {
        v.style = s.filters.stylesFilter([
          ['text-indent'],
          ['line-height'],
          [/^margin(:?-left)?$/, null, function (x) {
            var y = x.split(' ');
    ...
    for (var A = 0; A < y.length; A++) {
      ...
      if (z.name in CKEDITOR.dtd.$listItem) {
        ...
        s.filters.stylesFilter([
          ['tab-stops', null, function (H) {
        ...
        if (z.name in CKEDITOR.dtd.$list) {
    ...
    var v = CKEDITOR.dtd, w = CKEDITOR.tools.extend({}, v.$block, v.$listItem,
      v.$tableContent), x = u.config, y = this.filters, z = y.falsyFilter, A = y.stylesFilter, B = y.elementMigrateFilter, C = CKEDITOR.tools.bind(this.filters.styleMigrateFilter,
      this.filters), D = this.utils.createListBulletMarker, E = y.flattenList, F = y.assembleList, G = this.utils.isListBulletIndicator, H = this.utils.isContainingOnlySpaces, I = this.utils.resolveList, J = function (O) {
    ...
    return{elementNames:[
      ...
      if (P.match(/h\d/)) {
        ...
        if (P in v.$inline) {
          ...
          if (P.indexOf(':') != -1 && P.indexOf('cke') == -1) {
            O.filterChildren();
            if (P == 'v:imagedata') {
      ...
      if (W) {
        V.style = A([
          ['line-height'],
          [/^font-family$/, null, !M ? C(x.font_style, 'family') : null],
          [/^font-size$/, null, !M ? C(x.fontSize_style, 'size') : null],
          [/^color$/, null, !M ? C(x.colorButton_foreStyle, 'color') : null],
          [/^background-color$/, null, !M ? C(x.colorButton_backStyle, 'color') : null]
        ])(W, O) || '';
      }
      return null;
      ...
      if (P && !P.href && P.name) {
        ...
        if (CKEDITOR.env.webkit && P.href && P.href.match(/file:\/\/\/[\S]+#/i)) {
      ...
      [/(?:v|o):\w+/, ''],
      [/^lang/, '']
    ], attributes:{style:A(N ? [
      [/^list-style-type$/, null],
      [/^margin$|^margin-(?!bottom|top)/, null, function (O, P, Q) {
      ...
      [/^clear$/],
      [/^border.*|margin.*|vertical-align|float$/, null, function (O, P) {
        if (P.name == 'img') {
          return O;
        }
      }],
