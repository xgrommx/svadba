...
(function () {
  ...
  function iterate(rtl, breakOnFalse) {
    ...
    var node, range = this.range, guard, userGuard = this.guard, type = this.type, getSourceNodeFn = ( rtl ? 'getPreviousSourceNode' : 'getNextSourceNode' );
    ...
    if (!rtl && !this._.guardLTR) {
      ...
      var limitLTR = range.endContainer, blockerLTR = limitLTR.getChild(range.endOffset);
    ...
    if (rtl && !this._.guardRTL) {
      // Gets the node that stops the walker when going LTR.
      var limitRTL = range.startContainer, blockerRTL = ( range.startOffset > 0 ) && limitRTL.getChild(range.startOffset - 1);
    ...
    if (this.current) {
      ...
      if (rtl) {
        ...
        if (node) {
          ...
          node = ( guard(range.startContainer, true) === false ) ? null : range.startContainer.getNextSourceNode(true, type,
            guard);
        }
      }
    }
  ...
  CKEDITOR.dom.element.prototype.isBlockBoundary = function (customNodeNames) {
    var nodeNameMatches = customNodeNames ? CKEDITOR.tools.extend({}, CKEDITOR.dtd.$block,
      customNodeNames || {}) : CKEDITOR.dtd.$block;
  ...
  var tailNbspRegex = /^[\t\r\n ]*(?:&nbsp;|\xa0)$/, isWhitespaces = CKEDITOR.dom.walker.whitespaces(), isBookmark = CKEDITOR.dom.walker.bookmark(), toSkip = function (node) {
