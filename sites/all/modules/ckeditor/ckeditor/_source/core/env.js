...
if (!CKEDITOR.env) {
  ...
  CKEDITOR.env = (function () {
     ...
     */
    env.cssClass = 'cke_browser_' + (
      env.ie ? 'ie' : env.gecko ? 'gecko' : env.opera ? 'opera' : env.webkit ? 'webkit' : 'unknown' );
    ...
    if (env.ie) {
      env.cssClass += ' cke_browser_ie' + (
        env.version < 7 ? '6' : env.version >= 8 ? document.documentMode : '7' );
