...
( function () {
      ...
      if (element.filebrowser.action == 'Browse') {
        ...
        if (element.filebrowser.action == 'QuickUpload' && element[ 'for' ]) {
          ...
          if (url) {
            ...
            element.filebrowser.url = url;
            element.hidden = false;
            setupFileElement(editor, definition.getContents(element[ 'for' ][ 0 ]).get(element[ 'for' ][ 1 ]),
              element.filebrowser);
          }
        }
      }
  ...
  function setUrl(fileUrl, data) {
    var dialog = this._.filebrowserSe.getDialog(), targetInput = this._.filebrowserSe[ 'for' ], onSelect = this._.filebrowserSe.filebrowser.onSelect;
