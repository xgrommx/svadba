...
(function () {
  ...
  function convertInlineHandlers(container) {
    ...
    var children = container.getElementsByTag('*'), count = children.count(), child;
          ...
          (function (eventName) {
            ...
            if (node.hasAttribute('on' + eventName)) {
              ...
              node.on(eventName, function (evt) {
                var callFunc = /(return\s*)?CKEDITOR\.tools\.callFunction\(([^)]+)\)/.exec(inlineEventHandler), hasReturn = callFunc && callFunc[ 1 ], callFuncArgs = callFunc && callFunc[ 2 ].split(','), preventDefault = /return false;/.test(inlineEventHandler);

                if (callFuncArgs) {
                  var nums = callFuncArgs.length, argName;
  ...
  CKEDITOR.ui.on('ready', function (evt) {
    ...
    if (ui._.panel) {
      var panel = ui._.panel._.panel, holder;
      ...
      if (ui instanceof CKEDITOR.dialog) {
...
CKEDITOR.dom.document.prototype.write = CKEDITOR.tools.override(CKEDITOR.dom.document.prototype.write,
  function (original_write) {
    function appendElement(parent, tagName, fullTag, text) {
      var node = parent.append(tagName), attrs = CKEDITOR.htmlParser.fragment.fromHtml(fullTag).children[ 0 ].attributes;
    ...
    return function (html, mode) {
      ...
      if (this.getBody()) {
        ...
        var doc = this, head = this.getHead();
