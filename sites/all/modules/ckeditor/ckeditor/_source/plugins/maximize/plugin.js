...
(function () {
  ...
  function createIframeShim(element) {
    ...
    var shim = CKEDITOR.dom.element.createFromHtml('<iframe frameborder="0" tabindex="-1"' + ' src="javascript:' + 'void((function(){' + 'document.open();' + ( CKEDITOR.env.isCustomDomain() ? 'document.domain=\'' + this.getDocument().$.domain + '\';' : '' ) + 'document.close();' + '})())"' + ' style="display:block;position:absolute;z-index:-1;' + 'progid:DXImageTransform.Microsoft.Alpha(opacity=0);' + '"></iframe>');
        ...
        editor.addCommand('maximize', {
            ...
            exec:function () {
              ...
              if (this.state == CKEDITOR.TRISTATE_OFF)    // Go fullscreen if the state is off.
              {
                ...
                if (this.state == CKEDITOR.TRISTATE_ON)  // Restore from fullscreen if the state is on.
                {
