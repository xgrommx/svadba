...
(function () {
  ...
  CKEDITOR.plugins.enterkey = {
    enterBlock:function (editor, mode, range, forceMode) {
      ...
      var atBlockStart = range.checkStartOfBlock(), atBlockEnd = range.checkEndOfBlock(), path = new CKEDITOR.dom.elementPath(range.startContainer), block = path.block;
      ...
      // Don't split <pre> if we're in the middle of it, act as shift enter key.
        ...
        // Don't split caption blocks. (#7944)
          ...
          if (block && CKEDITOR.dtd.$captionBlock[ block.getName() ]) {
            enterBr(editor, mode, range, forceMode);
            return;
          }
        }
      ...
      if (!isStartOfBlock && !isEndOfBlock) {
        ...
        if (nextBlock.is('li') && ( node = nextBlock.getFirst(CKEDITOR.dom.walker.invisible(true)) ) && node.is && node.is('ul',
          'ol')) {
