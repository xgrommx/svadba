...
(function () {
  CKEDITOR.plugins.add('stylescombo', {
      ...
      init:function (editor) {
        var config = editor.config, lang = editor.lang.stylesCombo, styles = {}, stylesList = [], combo;
        ...
        editor.ui.addRichCombo('Styles', {
            ...
            onClick:function (value) {
              ...
              var style = styles[ value ], selection = editor.getSelection(), elementPath = new CKEDITOR.dom.elementPath(selection.getStartElement());
            ...
            onRender:function () {
              editor.on('selectionChange', function (ev) {
                  var currentValue = this.getValue(), elementPath = ev.data.path, elements = elementPath.elements;
            ...
            onOpen:function () {
              ...
              var selection = editor.getSelection(), element = selection.getSelectedElement(), elementPath = new CKEDITOR.dom.elementPath(element || selection.getStartElement()), counter = [ 0, 0, 0, 0 ];

              this.showAll();
              this.unmarkAll();
              for (var name in styles) {
                var style = styles[ name ], type = style.type;

                if (style.checkActive(elementPath)) {
                  this.mark(name);
                }
                else {
                  if (type == CKEDITOR.STYLE_OBJECT && !style.checkApplicable(elementPath)) {
                    this.hideItem(name);
                    counter[ type ]--;
                  }
                }
  ...
  function sortStyles(styleA, styleB) {
    var typeA = styleA.type, typeB = styleB.type;
