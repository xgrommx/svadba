...
(function () {
  ...
  var whitespaces = CKEDITOR.dom.walker.whitespaces(), bookmarks = CKEDITOR.dom.walker.bookmark(), nonEmpty = function (node) {
  ...
  CKEDITOR.plugins.list = {
    ...
    arrayToList:function (listArray, database, baseIndex, paragraphMode, dir) {
      ...
      var doc = listArray[ baseIndex ].parent.getDocument(), retval = new CKEDITOR.dom.documentFragment(doc), rootNode = null, currentIndex = baseIndex, indentLevel = Math.max(listArray[ baseIndex ].indent,
        0), currentListItem = null, orgDir, paragraphName = ( paragraphMode == CKEDITOR.ENTER_P ? 'p' : 'div' );
      while (1) {
        ...
        if (item.indent == indentLevel) {
          ...
          if (item.indent == Math.max(indentLevel, 0) + 1) {
            ...
            var currDir = listArray[ currentIndex - 1 ].element.getDirection(1), listData = CKEDITOR.plugins.list.arrayToList(listArray,
              null, currentIndex, paragraphMode, currDir != orgDir ? orgDir : null);
      ...
      if (database) {
        var currentNode = retval.getFirst(), listRoot = listArray[ 0 ].parent;
  ...
  function onSelectionChange(evt) {
    ...
    var path = evt.data.path, blockLimit = path.blockLimit, elements = path.elements, element, i;
  ...
  function changeListType(editor, groupObj, database, listsCreated) {
    ...
    var listArray = CKEDITOR.plugins.list.listToArray(groupObj.root, database), selectedListItems = [];
    ...
    var root = groupObj.root, fakeParent = root.getDocument().createElement(this.type);
  ...
  function removeList(editor, groupObj, database) {
    ...
    function compensateBrs(isStart) {
      if (( boundaryNode = docFragment[ isStart ? 'getFirst' : 'getLast' ]() ) && !( boundaryNode.is && boundaryNode.isBlockBoundary() ) && ( siblingNode = groupObj.root[ isStart ? 'getPrevious' : 'getNext' ](CKEDITOR.dom.walker.whitespaces(true)) ) && !( siblingNode.is && siblingNode.isBlockBoundary({ br:1 }) ))
    ...
    exec:function (editor) {
      ...
      if (this.state == CKEDITOR.TRISTATE_OFF) {
        ...
        if (!body.getFirst(nonEmpty)) {
          config.enterMode == CKEDITOR.ENTER_BR ? body.appendBogus() : ranges[ 0 ].fixBlock(1,
            config.enterMode == CKEDITOR.ENTER_P ? 'p' : 'div');
      ...
      while (( range = rangeIterator.getNextRange() ) && ++index) {
        ...
        while (( block = iterator.getNextParagraph() )) {
          ...
          var path = new CKEDITOR.dom.elementPath(block), pathElements = path.elements, pathElementsCount = pathElements.length, listNode = null, processedFlag = 0, blockLimit = path.blockLimit, element;
  ...
  function getExtendNestedListFilter(isHtmlFilter) {
    ...
    return function (listItem) {
      var children = listItem.children, firstNestedListIndex = indexOfFirstChildElement(listItem,
        dtd.$list), firstNestedList = children[ firstNestedListIndex ], nodeBefore = firstNestedList && firstNestedList.previous, tailNbspmatch;
  ...
  CKEDITOR.plugins.add('list', {
      init:function (editor) {
        // Register commands.
        var numberedListCommand = editor.addCommand('numberedlist',
          new listCommand('numberedlist', 'ol')), bulletedListCommand = editor.addCommand('bulletedlist',
          new listCommand('bulletedlist', 'ul'));
