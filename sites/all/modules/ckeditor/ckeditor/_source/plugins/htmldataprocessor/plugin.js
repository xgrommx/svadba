...
(function () {
  ...
  function lastNoneSpaceChild(block) {
    var lastIndex = block.children.length, last = block.children[ lastIndex - 1 ];
  ...
  function blockNeedsExtension(block, fromSource, extendEmptyBlock) {
    ...
    return !lastChild || lastChild && ( lastChild.type == CKEDITOR.NODE_ELEMENT && lastChild.name == 'br'
      ...
      // to move cursor at the end of the form. (#4791) || block.name == 'form' && lastChild.name == 'input' );
  }
    ...
    elements:{
      ...
      table:function (element) {
        ...
        children.sort(function (node1, node2) {
          return node1.type == CKEDITOR.NODE_ELEMENT && node2.type == node1.type ? CKEDITOR.tools.indexOf(tableOrder,
            node1.name) > CKEDITOR.tools.indexOf(tableOrder, node2.name) ? 1 : -1 : 0;
        });
      },
      ...
      embed:function (element) {
        ...
        if (parent && parent.name == 'object') {
          var parentWidth = parent.attributes.width, parentHeight = parent.attributes.height;
  ...
  var protectElementRegex = /<(a|area|img|input)\b([^>]*)>/gi, protectAttributeRegex = /\b(on\w+|href|src|name)\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|(?:[^ "'>]+))/gi;
  ...
  function unprotectSource(html, editor) {
    ...
    return html.replace(/<!--\{cke_protected\}([\s\S]+?)-->/g,
      function (match, data) {
        return decodeURIComponent(data);
  ...
  function protectSource(data, editor) {
    var protectedHtml = [], protectRegexes = editor.config.protectedSource, store = editor._.dataStore || ( editor._.dataStore = { id:1 } ), tempRegex = /<\!--\{cke_temp(comment)?\}(\d*?)-->/g;
    ...
    data = data.replace(tempRegex, function ($, isComment, id) {
        return '<!--' + protectedSourceMarker + ( isComment ? '{C}' : '' ) + encodeURIComponent(protectedHtml[ id ]).replace(/--/g,
          '%2D%2D') + '-->';
      });
  ...
  CKEDITOR.htmlDataProcessor.prototype = {
    toHtml:function (data, fixForBody) {
      ...
      var fragment = CKEDITOR.htmlParser.fragment.fromHtml(data, fixForBody), writer = new CKEDITOR.htmlParser.basicWriter();
    ...
    toDataFormat:function (html, fixForBody) {
      var writer = this.writer, fragment = CKEDITOR.htmlParser.fragment.fromHtml(html, fixForBody);
