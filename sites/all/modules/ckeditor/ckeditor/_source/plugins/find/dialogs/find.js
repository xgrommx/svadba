...
(function () {
  ...
  function nonCharactersBoundary(node) {
    return !( node.type == CKEDITOR.NODE_ELEMENT && node.isBlockBoundary(CKEDITOR.tools.extend({}, CKEDITOR.dtd.$empty,
  ...
  var pages = [ 'find', 'replace' ], fieldsMapping = [
  ...
  var findDialog = function (editor, startupPage) {
    ...
    var highlightStyle = new CKEDITOR.style(CKEDITOR.tools.extend({ attributes:{ 'data-cke-highlight':1 }, fullMatch:1, ignoreReadonly:1, childRule:function () {
        return 0;
    ...
    characterWalker.prototype = {
      ...
      move:function (rtl) {
        ...
        if (currentTextNode && rtl && this.offset > 0) {
          ...
          if (currentTextNode && this.offset < currentTextNode.getLength() - 1) {
            this.offset++;
            return cursorStep.call(this);
          }
          else {
    ...
    characterRange.prototype = {
      ...
      toDomRange:function () {
        ...
        if (cursors.length < 1) {
          ...
          var first = cursors[0], last = cursors[ cursors.length - 1 ];
      ...
      updateFromDomRange:function (domRange) {
        var cursor, walker = new characterWalker(domRange);
        this._.cursors = [];
        do {
          cursor = walker.next();
          if (cursor.character) {
      ...
      moveBack:function () {
        var retval = this._.walker.back(), cursors = this._.cursors;
      ...
      moveNext:function () {
        var retval = this._.walker.next(), cursors = this._.cursors;
      ...
      getNextCharacterRange:function (maxLength) {
        var lastCursor, nextRangeWalker, cursors = this._.cursors;
    ...
    var finder = {
      ...
      find:function (pattern, matchCase, matchWord, matchCyclic, highlightMatched, cyclicRerun) {
        ...
        while (character !== null) {
          ...
          if (matchState == KMP_MATCHED) {
            if (matchWord) {
              var cursors = this.matchRange.getCursors(), tail = cursors[ cursors.length - 1 ], head = cursors[ 0 ];

              var headWalker = new characterWalker(getRangeBeforeCursor(head),
                true), tailWalker = new characterWalker(getRangeAfterCursor(tail), true);
    ...
    function getSearchRange(isDefault) {
      var searchRange, sel = editor.getSelection(), body = editor.document.getBody();
      if (sel && !isDefault) {
              ...
              children:[
                ...
                },
                {
                  ...
                  onClick:function () {
                    ...
                    if (!finder.find(dialog.getValueOf('find', 'txtFindFind'), dialog.getValueOf('find', 'txtFindCaseChk'),
                      dialog.getValueOf('find', 'txtFindWordChk'), dialog.getValueOf('find', 'txtFindCyclic'))) {
          ...
          elements:[
            {
              ...
              children:[
                ...
                },
                {
                  ...
                  onClick:function () {
                    var dialog = this.getDialog();
                    if (!finder.replace(dialog, dialog.getValueOf('replace', 'txtFindReplace'),
                      dialog.getValueOf('replace', 'txtReplace'), dialog.getValueOf('replace', 'txtReplaceCaseChk'),
                      dialog.getValueOf('replace', 'txtReplaceWordChk'), dialog.getValueOf('replace', 'txtReplaceCyclic'))) {
                      alert(lang.notFoundMsg);
                    }
                  }
                }
              ...
              children:[
                ...
                },
                {
                  ...
                  onClick:function () {
                    ...
                    editor.fire('saveSnapshot');
                    while (finder.replace(dialog, dialog.getValueOf('replace', 'txtFindReplace'),
                      dialog.getValueOf('replace', 'txtReplace'), dialog.getValueOf('replace', 'txtReplaceCaseChk'),
                      dialog.getValueOf('replace', 'txtReplaceWordChk'), false, true)) { /*jsl:pass*/
                    }
