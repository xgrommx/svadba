...
CKEDITOR.plugins.add('menu', {
    beforeInit:function (editor) {
      var groups = editor.config.menu_groups.split(','), groupsOrder = editor._.menuGroups = {}, menuItems = editor._.menuItems = {};
...
(function () {
  CKEDITOR.menu = CKEDITOR.tools.createClass({
      ...
      _:{
        onShow:function () {
          ...
          var element = selection && selection.getStartElement(), listeners = this._.listeners, includedItems = [];
        ...
        onEscape:function (keystroke) {
          ...
          if (parent) {
            ...
            var parentBlock = parent._.panel._.panel._.currentBlock, parentFocusIndex = parentBlock._.focusIndex;
        ...
        showSubMenu:function (index) {
          var menu = this._.subMenu, item = this.items[ index ], subItemDefs = item.getItems && item.getItems();
        ...
        show:function (offsetParent, corner, offsetX, offsetY) {
          ...
          if (!panel) {
            panel = this._.panel = new CKEDITOR.ui.floatPanel(this.editor, CKEDITOR.document.getBody(),
              this._.panelDefinition, this._.level);
            ...
            this._.itemOverFn = CKEDITOR.tools.addFunction(function (index) {
                ...
                this._.showSubTimeout = CKEDITOR.tools.setTimeout(this._.showSubMenu, editor.config.menu_subMenuDelay || 400,
                  this, [ index ]);
              }, this);
          ...
          var chromeRoot = editor.container.getChild(1), mixedContentClass = chromeRoot.hasClass('cke_mixed_dir_content') ? ' cke_mixed_dir_content' : '';
  ...
  CKEDITOR.menuItem = CKEDITOR.tools.createClass({
      $:function (editor, name, definition) {
        CKEDITOR.tools.extend(this, definition, // Defaults
          {
            order:0,
      ...
      proto:{
        render:function (menu, index, output) {
          var id = menu.id + String(index), state = ( typeof this.state == 'undefined' ) ? CKEDITOR.TRISTATE_OFF : this.state;

          var classes = ' cke_' + (
            state == CKEDITOR.TRISTATE_ON ? 'on' : state == CKEDITOR.TRISTATE_DISABLED ? 'disabled' : 'off' );
          ...
          output.push('<span class="cke_menuitem' + ( this.icon && this.icon.indexOf('.png') == -1 ? ' cke_noalphafix' : '' ) + '">' + '<a id="',
            id, '"' + ' class="', classes, '" href="javascript:void(\'', ( this.label || '' ).replace("'", ''),
            '\')"' + ' title="', this.label,
            '"' + ' tabindex="-1"' + '_cke_focus=1' + ' hidefocus="true"' + ' role="menuitem"' + ( hasSubMenu ? 'aria-haspopup="true"' : '' ) + ( state == CKEDITOR.TRISTATE_DISABLED ? 'aria-disabled="true"' : '' ) + ( state == CKEDITOR.TRISTATE_ON ? 'aria-pressed="true"' : '' ));
          ...
          output.push(//					' onkeydown="return CKEDITOR.ui.button._.keydown(', index, ', event);"' +
            ...
            '="CKEDITOR.tools.callFunction(', menu._.itemClickFn, ',', index,
            '); return false;"' + '>' + '<span class="cke_icon_wrapper"><span class="cke_icon"' + ( this.icon ? ' style="background-image:url(' + CKEDITOR.getUrl(this.icon) + ');background-position:0 ' + offset + 'px;"' : '' ) + '></span></span>' + '<span class="cke_label">');

          if (hasSubMenu) {
            output.push('<span class="cke_menuarrow">', '<span>&#',
              ( this.editor.lang.dir == 'rtl' ? '9668' : // BLACK LEFT-POINTING POINTER
                '9658' ), // BLACK RIGHT-POINTING POINTER
              ';</span>', '</span>');
          }

          output.push(htmlLabel, '</span>' + '</a>' + '</span>');
        }
      }
    });
