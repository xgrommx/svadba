...
(function () {
  ...
  function getPreviousVisibleTab() {
    var tabId = this._.currentTabId, length = this._.tabIdList.length, tabIndex = CKEDITOR.tools.indexOf(this._.tabIdList,
      tabId) + length;
  ...
  CKEDITOR.dialog = function (editor, dialogName) {
    ...
    var definition = CKEDITOR.dialog._.dialogDefinitions[ dialogName ], defaultDefinition = CKEDITOR.tools.clone(defaultDialogDefinition), buttonsOrder = editor.config.dialog_buttonsOrder || 'OS', dir = editor.lang.dir;
    ...
    var iterContents = function (func) {
      var contents = me._.contents, stop = false;
    ...
    this.on('ok', function (evt) {
      iterContents(function (item) {
        if (item.validate) {
          var retval = item.validate(this), invalid = typeof ( retval ) == 'string' || retval === false;
    ...
    function changeFocus(forward) {
      var focusList = me._.focusList, offset = forward ? 1 : -1;
      ...
      var startIndex = ( current + offset + focusList.length ) % focusList.length, currentIndex = startIndex;
    ...
    function focusKeydownHandler(evt) {
      ...
      var keystroke = evt.data.getKeystroke(), rtl = editor.lang.dir == 'rtl';
      ...
      if (keystroke == 9 || keystroke == CKEDITOR.SHIFT + 9) {
        ...
        if (keystroke == CKEDITOR.ALT + 121 && !me._.tabBarMode && me.getPageCount() > 1) {
          ...
          if (( keystroke == 37 || keystroke == 39 ) && me._.tabBarMode) {
            ...
            if (( keystroke == 13 || keystroke == 32 ) && me._.tabBarMode) {
      ...
      if (editor.config.dialog_startupFocusTab && me._.pageCount > 1) {
        ...
        if (!this._.hasFocus) {
          ...
          if (this._.editor.mode == 'wysiwyg' && CKEDITOR.env.ie) {
            var $selection = editor.document.$.selection, $range = $selection.createRange();
    ...
    if (CKEDITOR.env.ie6Compat) {
      this.on('load', function (evt) {
        var outer = this.getElement(), inner = outer.getFirst();
        inner.remove();
    ...
    // Insert buttons.
    var buttonsHtml = [], buttons = CKEDITOR.dialog._.uiElementBuilders.hbox.build(this, {
        type:'hbox',
  ...
  CKEDITOR.dialog.prototype = {
     ...
     */
    layout:function () {
      var viewSize = CKEDITOR.document.getWindow().getViewPaneSize(), dialogSize = this.getSize();
    ...
    hide:function () {
      ...
      if (!this._.parentDialog) {
        ...
        var parentElement = this._.parentDialog.getElement().getFirst();
        parentElement.setStyle('z-index',
          parseInt(parentElement.$.style.zIndex, 10) + Math.floor(this._.editor.config.baseFloatZIndex / 2));
      }
    ...
    addPage:function (contents) {
      var pageHtml = [], titleHtml = contents.label ? ' title="' + CKEDITOR.tools.htmlEncode(contents.label) + '"' : '', elements = contents.elements, vbox = CKEDITOR.dialog._.uiElementBuilders.vbox.build(this,
        {
          type:'vbox',
      ...
      var tabId = 'cke_' + contents.id + '_' + CKEDITOR.tools.getNextNumber(), tab = CKEDITOR.dom.element.createFromHtml([
        '<a class="cke_dialog_tab"', ( this._.pageCount > 0 ? ' cke_last' : 'cke_first' ), titleHtml, ( !!contents.hidden ? ' style="display:none"' : '' ), ' id="', tabId, '"', env.gecko && env.version >= 10900 && !env.hc ? '' : ' href="javascript:void(0)"', ' tabIndex="-1"', ' hidefocus="true"', ' role="tab">', contents.label, '</a>'
      ].join(''));
      ...
      var contentMap = this._.contents[ contents.id ] = {}, cursor, children = vbox.getChild();
    ...
    selectPage:function (id) {
      ...
      for (var i in this._.tabs) {
        var tab = this._.tabs[i][0], page = this._.tabs[i][1];
        if (i != id) {
  ...
  definitionObject.prototype = /** @lends CKEDITOR.dialog.definitionObject.prototype */
  {
    /**
  ...
  contentObject.prototype = /** @lends CKEDITOR.dialog.definition.contentObject.prototype */
  {
    /**
  ...
  function initDragAndDrop(dialog) {
    var lastCoords = null, abstractDialogCoords = null, element = dialog.getElement().getFirst(), editor = dialog.getParentEditor(), magnetDistance = editor.config.dialog_magnetDistance, margins = editor.skin.margins || [ 0, 0, 0, 0 ];
    ...
    function mouseMoveHandler(evt) {
      var dialogSize = dialog.getSize(), viewPaneSize = CKEDITOR.document.getWindow().getViewPaneSize(), x = evt.data.$.screenX, y = evt.data.$.screenY, dx = x - lastCoords.x, dy = y - lastCoords.y, realX, realY;
  ...
  function initResizeHandles(dialog) {
    var def = dialog.definition, resizable = def.resizable;
    ...
    var mouseDownFn = CKEDITOR.tools.addFunction(function ($event) {
      ...
      wrapperHeight = startSize.height - dialog.parts.contents.getSize('height',
    ...
    dialog.on('load', function () {
      ...
      var resizer = CKEDITOR.dom.element.createFromHtml('<div' + ' class="cke_resizer' + direction + ' cke_resizer_' + editor.lang.dir + '"' + ' title="' + CKEDITOR.tools.htmlEncode(editor.lang.resize) + '"' + ' onmousedown="CKEDITOR.tools.callFunction(' + mouseDownFn + ', event )"></div>');
    ...
    function mouseMoveHandler(evt) {
      var rtl = editor.lang.dir == 'rtl', dx = ( evt.data.$.screenX - origin.x ) * ( rtl ? -1 : 1 ), dy = evt.data.$.screenY - origin.y, width = startSize.width, height = startSize.height, internalWidth = width + dx * ( dialog._.moved ? 1 : 2 ), internalHeight = height + dy * ( dialog._.moved ? 1 : 2 ), element = dialog._.element.getFirst(), right = rtl && element.getComputedStyle('right'), position = dialog.getPosition();
  ...
  function showCover(editor) {
    ...
    var config = editor.config, backgroundColorStyle = config.dialog_backgroundCoverColor || 'white', backgroundCoverOpacity = config.dialog_backgroundCoverOpacity, baseFloatZIndex = config.baseFloatZIndex, coverKey = CKEDITOR.tools.genKey(backgroundColorStyle,
      backgroundCoverOpacity, baseFloatZIndex), coverElement = covers[ coverKey ];

    if (!coverElement) {
      var html = [
        '<div tabIndex="-1" style="position: ', ( CKEDITOR.env.ie6Compat ? 'absolute' : 'fixed' ), '; z-index: ', baseFloatZIndex, '; top: 0px; left: 0px; ', ( !CKEDITOR.env.ie6Compat ? 'background-color: ' + backgroundColorStyle : '' ), '" class="cke_dialog_background_cover">'
      ];

      if (CKEDITOR.env.ie6Compat) {
        // Support for custom document.domain in IE.
        var isCustomDomain = CKEDITOR.env.isCustomDomain(), iframeHtml = '<html><body style=\\\'background-color:' + backgroundColorStyle + ';\\\'></body></html>';
        ...
        html.push('"' + ' style="' + 'position:absolute;' + 'left:0;' + 'top:0;' + 'width:100%;' + 'height: 100%;' + 'progid:DXImageTransform.Microsoft.Alpha(opacity=0)">' + '</iframe>');
      }
    ...
    var scrollFunc = function () {
      var pos = win.getScrollPosition(), cursor = CKEDITOR.dialog._.currentTop;
      coverElement.setStyles({
          left:pos.x + 'px',
          top:pos.y + 'px'
        });
  ...
  var accessKeyDownHandler = function (evt) {
    var ctrl = evt.data.$.ctrlKey || evt.data.$.metaKey, alt = evt.data.$.altKey, shift = evt.data.$.shiftKey, key = String.fromCharCode(evt.data.$.keyCode), keyProcessor = accessKeyProcessors[( ctrl ? 'CTRL+' : '' ) + ( alt ? 'ALT+' : '') + ( shift ? 'SHIFT+' : '' ) + key];
    ...
    CKEDITOR.ui.dialog = {
      ...
      hbox:function (dialog, childObjList, childHtmlList, htmlList, elementDefinition) {
        ...
        var innerHTML = function () {
          ...
          for (i = 0; i < childHtmlList.length; i++) {
            var className = 'cke_dialog_ui_hbox_child', styles = [];
            if (i === 0) {
      ...
      vbox:function (dialog, childObjList, childHtmlList, htmlList, elementDefinition) {
        ...
        var innerHTML = function () {
          ...
          html.push('"');
          html.push('align="',
            CKEDITOR.tools.htmlEncode(( elementDefinition && elementDefinition.align ) || ( dialog.getParentEditor().lang.dir == 'ltr' ? 'left' : 'right' )),
            '" ');
        ...
        CKEDITOR.ui.dialog.uiElement.call(this, dialog, elementDefinition || { type:'vbox' }, htmlList, 'div', null,
          { role:'presentation' }, innerHTML);
      }
    };
  })();
  ...
  CKEDITOR.ui.dialog.uiElement.prototype = {
    ...
    selectParentTab:function () {
      var element = this.getInputElement(), cursor = element, tabId;
     ...
     */
    disable:function () {
      var element = this.getElement(), input = this.getInputElement();
   ...
   */
    {
      ...
      getChild:function (indices) {
        ...
        if (indices.length < 2) {
          ...
          return ( this._.children[ indices[0] ] && this._.children[ indices[0] ].getChild ) ? this._.children[ indices[0] ].getChild(indices.slice(1,
            indices.length)) : null;
        }
      }
    }, true);
  ...
  (function () {
    var commonBuilder = {
      build:function (dialog, elementDefinition, output) {
        var children = elementDefinition.children, child, childHtmlList = [], childObjList = [];
        for (var i = 0; ( i < children.length && ( child = children[i] ) ); i++) {
          var childHtml = [];
          childHtmlList.push(childHtml);
          childObjList.push(CKEDITOR.dialog._.uiElementBuilders[ child.type ].build(dialog, child, childHtml));
        }
        return new CKEDITOR.ui.dialog[elementDefinition.type](dialog, childObjList, childHtmlList, output,
          elementDefinition);
      }
    };
  ...
  (function () {
    var notEmptyRegex = /^([a]|[^a])+$/, integerRegex = /^\d*$/, numberRegex = /^\d*(?:\.\d+)?$/, htmlLengthRegex = /^(((\d*(\.\d+))|(\d*))(px|\%)?)?$/, cssLengthRegex = /^(((\d*(\.\d+))|(\d*))(px|em|ex|in|cm|mm|pt|pc|\%)?)?$/i, inlineStyleRegex = /^(\s*[\w-]+\s*:\s*[^:;]+(?:;|$))*$/;
  ...
  CKEDITOR.tools.extend(CKEDITOR.editor.prototype, /** @lends CKEDITOR.editor.prototype */
    {
      /**
      ...
      openDialog:function (dialogName, callback) {
        ...
        var dialogDefinitions = CKEDITOR.dialog._.dialogDefinitions[ dialogName ], dialogSkin = this.skin.dialog;
        ...
        if (typeof dialogDefinitions == 'function' && dialogSkin._isLoaded) {
          ...
          var dialog = storedDialogs[ dialogName ] || ( storedDialogs[ dialogName ] = new CKEDITOR.dialog(this,
            dialogName) );
        ...
        function onDialogFileLoaded(success) {
          var dialogDefinition = CKEDITOR.dialog._.dialogDefinitions[ dialogName ], skin = me.skin.dialog;
