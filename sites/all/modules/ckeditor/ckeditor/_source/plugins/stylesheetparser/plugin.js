...
(function () {
  ...
  function parseClasses(aRules, skipSelectors, validSelectors) {
    ...
    var aSelectors = s.split(' '), aClasses = [];
  ...
  function LoadStylesCSS(theDoc, skipSelectors, validSelectors) {
    var styles = [], // It will hold all the rules of the applied stylesheets (except those internal to CKEditor)
      aRules = [], i;

    for (i = 0; i < theDoc.styleSheets.length; i++) {
      var sheet = theDoc.styleSheets[ i ], node = sheet.ownerNode || sheet.owningElement;
    ...
    for (i = 0; i < aClasses.length; i++) {
      var oElement = aClasses[ i ].split('.'), element = oElement[ 0 ].toLowerCase(), sClassName = oElement[ 1 ];
        ...
        obj.getStylesSet = CKEDITOR.tools.override(obj.getStylesSet, function (org) {
          return function (callback) {
            ...
            org.call(this, function (definitions) {
              ...
              var skipSelectors = self.config.stylesheetParser_skipSelectors || ( /(^body\.|^\.)/i ), // Rules that are valid
                validSelectors = self.config.stylesheetParser_validSelectors || ( /\w+\.\w+/ );

              callback(( self._.stylesDefinitions = definitions.concat(LoadStylesCSS(self.document.$, skipSelectors,
                validSelectors)) ));
            });
          };
        });
