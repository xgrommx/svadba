...
(function () {
  CKEDITOR.on('dialogDefinition', function (ev) {
    var tab, name = ev.data.name, definition = ev.data.definition;
  ...
  var bbcodeMap = { 'b':'strong', 'u':'u', 'i':'em', 'color':'span', 'size':'span', 'quote':'blockquote', 'code':'code', 'url':'a', 'email':'span', 'img':'span', '*':'li', 'list':'ol' }, convertMap = { 'strong':'b', 'b':'b', 'u':'u', 'em':'i', 'i':'i', 'code':'code', 'li':'*' }, tagnameMap = { 'strong':'b', 'em':'i', 'u':'u', 'li':'*', 'ul':'list', 'ol':'list', 'code':'code', 'a':'link', 'img':'img', 'blockquote':'quote' }, stylesMap = { 'color':'color', 'size':'font-size' }, attributesMap = { 'url':'href', 'email':'mailhref', 'quote':'cite', 'list':'listType' };

  // List of block-like tags.
  var dtd = CKEDITOR.dtd, blockLikeTags = CKEDITOR.tools.extend({ table:1 }, dtd.$block, dtd.$listItem, dtd.$tableContent,
    dtd.$list);
  ...
  function serializeStyleText(stylesObject) {
    ...
    for (var style in stylesObject) {
      var styleVal = stylesObject[ style ], text = ( style + ':' + styleVal ).replace(semicolonFixRegex, ';');
  ...
  var smileyMap = {"smiley":":)", "sad":":(", "wink":";)", "laugh":":D", "cheeky":":P", "blush":":*)", "surprise":":-o", "indecision":":|", "angry":">:(", "angel":"o:)", "cool":"8-)", "devil":">:-)", "crying":";(", "kiss":":-*" }, smileyReverseMap = {}, smileyRegExp = [];
  ...
  CKEDITOR.BBCodeParser.prototype = {
    parse:function (bbcode) {
      ...
      while (( parts = this._.bbcPartsRegex.exec(bbcode) )) {
        ...
        // Opening tag
        if (parts[ 1 ]) {
          var tagName = bbcodeMap[ part ], attribs = {}, styles = {}, optionPart = parts[ 2 ];
  ...
  CKEDITOR.htmlParser.fragment.fromBBCode = function (source) {
    var parser = new CKEDITOR.BBCodeParser(), fragment = new CKEDITOR.htmlParser.fragment(), pendingInline = [], pendingBrs = 0, currentNode = fragment, returnPoint;

    function checkPending(newTagName) {
      if (pendingInline.length > 0) {
        for (var i = 0; i < pendingInline.length; i++) {
          var pendingElement = pendingInline[ i ], pendingName = pendingElement.name, pendingDtd = CKEDITOR.dtd[ pendingName ], currentDtd = currentNode.name && CKEDITOR.dtd[ currentNode.name ];
    ...
    function checkPendingBrs(tagName, closing) {
      var len = currentNode.children.length, previous = len > 0 && currentNode.children[ len - 1 ], lineBreakParent = !previous && BBCodeWriter.getRule(tagnameMap[ currentNode.name ],
        'breakAfterOpen'), lineBreakPrevious = previous && previous.type == CKEDITOR.NODE_ELEMENT && BBCodeWriter.getRule(tagnameMap[ previous.name ],
        'breakAfterClose'), lineBreakCurrent = tagName && BBCodeWriter.getRule(tagnameMap[ tagName ],
        closing ? 'breakBeforeClose' : 'breakBeforeOpen');
    ...
    function addElement(node, target) {
      ...
      var len = target.children.length, previous = len > 0 && target.children[ len - 1 ] || null;
    ...
    parser.onTagOpen = function (tagName, attributes, selfClosing) {
      ...
      if (currentDtd && !currentDtd[ tagName ]) {
        var reApply = false, addPoint;   // New position to start adding nodes.
    ...
    while (currentNode.type) {
      var parent = currentNode.parent, node = currentNode;
  ...
  CKEDITOR.plugins.add('bbcode', {
      ...
      init:function (editor) {
        ...
        dataFilter.addRules({
            elements:{
              ...
              'smiley':function (element) {
                ...
                var description = element.attributes.desc, image = config.smiley_images[ CKEDITOR.tools.indexOf(config.smiley_descriptions,
                  description) ], src = CKEDITOR.tools.htmlEncode(config.smiley_path + image);
        ...
        editor.dataProcessor.htmlFilter.addRules({
            elements:{
              $:function (element) {
                var attributes = element.attributes, style = parseStyleText(attributes.style), value;
                  ...
                  if (tagName == 'span') {
                    ...
                    if (tagName == 'ol' || tagName == 'ul') {
                      ...
                      if (tagName == 'blockquote') {
                        try {
                          var cite = element.children[ 0 ], quoted = element.children[ 1 ], citeText = cite.name == 'cite' && cite.children[ 0 ].value;
        ...
        if (editor._.elementsPath) {
          ...
          if (( filters = editor._.elementsPath.filters ))
            filters.push(function (element) {
              var htmlName = element.getName(), name = tagnameMap[ htmlName ] || false;
