...
(function () {
  var blockElements = { address:1, div:1, h1:1, h2:1, h3:1, h4:1, h5:1, h6:1, p:1, pre:1, section:1, header:1, footer:1, nav:1, article:1, aside:1, figure:1, dialog:1, hgroup:1, time:1, meter:1, menu:1, command:1, keygen:1, output:1, progress:1, details:1, datagrid:1, datalist:1 }, objectElements = { a:1, embed:1, hr:1, img:1, li:1, object:1, ol:1, table:1, td:1, tr:1, th:1, ul:1, dl:1, dt:1, dd:1, form:1, audio:1, video:1 };
  ...
  CKEDITOR.style.prototype = {
    ...
    applyToRange:function (range) {
      return ( this.applyToRange = this.type == CKEDITOR.STYLE_INLINE ? applyInlineStyle : this.type == CKEDITOR.STYLE_BLOCK ? applyBlockStyle : this.type == CKEDITOR.STYLE_OBJECT ? applyObjectStyle : null ).call(this,
        range);
    },

    removeFromRange:function (range) {
      return ( this.removeFromRange = this.type == CKEDITOR.STYLE_INLINE ? removeInlineStyle : this.type == CKEDITOR.STYLE_BLOCK ? removeBlockStyle : this.type == CKEDITOR.STYLE_OBJECT ? removeObjectStyle : null ).call(this,
        range);
    },
      ...
      if (typeof this.element == 'string' ? name == this.element : name in this.element) {
        ...
        if (attribs._length) {
          for (var attName in attribs) {
            ...
            if (attName == 'style' ? compareCssText(attribs[ attName ],
              normalizeCssText(elementAttr, false)) : attribs[ attName ] == elementAttr) {
              if (!fullMatch) {
                return true;
              }
            }
    ...
    buildPreview:function (label) {
      var styleDefinition = this._.definition, html = [], elementName = styleDefinition.element;
  ...
  function removeInlineStyle(range) {
    ...
    if (range.collapsed) {
      ...
      var startPath = new CKEDITOR.dom.elementPath(startNode.getParent()), // The topmost element in elementspatch which we should jump out of.
        boundaryElement;
      ...
      for (var i = 0, element; i < startPath.elements.length && ( element = startPath.elements[i] ); i++) {
        ...
        if (this.checkElementRemovable(element)) {
          ...
          if (range.collapsed && (
            range.checkBoundaryOfElement(element, CKEDITOR.END) || ( isStart = range.checkBoundaryOfElement(element,
              CKEDITOR.START) ) )) {
       ...
       */
      function breakNodes() {
        var startPath = new CKEDITOR.dom.elementPath(startNode.getParent()), endPath = new CKEDITOR.dom.elementPath(endNode.getParent()), breakStart = null, breakEnd = null;
  ...
  function splitIntoPres(preBlock) {
    ...
    var duoBrRegex = /(\S\s*)\n(?:\s|(<span[^>]+data-cke-bookmark.*?\/span>))*\n(?!$)/gi, blockName = preBlock.getName(), splitedHtml = replace(preBlock.getOuterHtml(),
      duoBrRegex, function (match, charBefore, bookmark) {
  ...
  function removeFromElement(style, element) {
    var def = style._.definition, attributes = CKEDITOR.tools.extend({}, def.attributes,
      getOverrides(style)[ element.getName() ]), styles = def.styles, // If the style is only about the element itself, we have to remove the element.
      removeEmpty = CKEDITOR.tools.isEmpty(attributes) && CKEDITOR.tools.isEmpty(styles);

    // Remove definition attributes/style from the elemnt.
    for (var attName in attributes) {
      // The 'class' element value must match (#1318).
      if (( attName == 'class' || style._.definition.fullMatch ) && element.getAttribute(attName) != normalizeProperty(attName,
        attributes[ attName ])) {
        continue;
      }
    ...
    for (var styleName in styles) {
      ...
      if (style._.definition.fullMatch && element.getStyle(styleName) != normalizeProperty(styleName, styles[ styleName ],
        true)) {
        continue;
      }
  ...
  function removeFromInsideElement(style, element) {
    var def = style._.definition, attribs = def.attributes, styles = def.styles, overrides = getOverrides(style), innerElements = element.getElementsByTag(style.element);
  ...
  function removeNoAttribsElement(element) {
    ...
    if (!element.hasAttributes()) {
      if (CKEDITOR.dtd.$block[ element.getName() ]) {
        var previous = element.getPrevious(nonWhitespaces), next = element.getNext(nonWhitespaces);
  ...
  function getElement(style, targetDocument, element) {
    var el, def = style._.definition, elementName = style.element;
  ...
  function setupElement(el, style) {
    var def = style._.definition, attributes = def.attributes, styles = CKEDITOR.style.getStyleText(def);
  ...
  function normalizeCssText(unparsedCssText, nativeNormalize) {
    ...
    return styleText.replace(/\s*([;:])\s*/, '$1').replace(/([^\s;])$/,
      '$1;')// Trimming spaces after comma(#4107),// remove quotations(#6403),// mostly for differences on "font-family"..replace(/,\s+/g,
  ...
  function applyStyle(document, remove) {
    var selection = document.getSelection(), // Bookmark the range so we can re-select it after processing.
      bookmarks = selection.createBookmarks(1), ranges = selection.getRanges(), func = remove ? this.removeFromRange : this.applyToRange, range;
...
CKEDITOR.editor.prototype.getStylesSet = function (callback) {
  if (!this._.stylesDefinitions) {
    ...
    CKEDITOR.stylesSet.addExternal(styleSetName,
      externalPath ? partsStylesSet.slice(1).join(':') : pluginPath + 'styles/' + styleSetName + '.js', '');
