...
(function () {
  ...
  CKEDITOR.plugins.add('selection', {
      init:function (editor) {
        ...
        editor.on('contentDom', function () {
          var doc = editor.document, body = doc.getBody(), html = doc.getDocumentElement();
          ...
          if (CKEDITOR.env.ie) {
            ...
            function saveSelection(testIt) {
              if (saveEnabled) {
                var doc = editor.document, sel = editor.getSelection(), nativeSel = sel && sel.getNative();
  ...
  CKEDITOR.dom.selection.prototype = {
     ...
     */
    getNative:CKEDITOR.env.ie ? function () {
    ...
    getType:CKEDITOR.env.ie ? function () {
      ...
      try {
        var sel = this.getNative(), ieType = sel.type;
    ...
    getRanges:(function () {
      var func = CKEDITOR.env.ie ? ( function () {
        ...
        var getBoundaryInformation = function (range, start) {
          ...
          var siblings = parent.children, child, sibling, testRange = range.duplicate(), startIndex = 0, endIndex = siblings.length - 1, index = -1, position, distance;
        ...
        return function () {
          ...
          var sel = this.getNative(), nativeRange = sel && sel.createRange(), type = this.getType(), range;
        ...
        var ranges = [], range, doc = this.document, sel = this.getNative();
    ...
    getSelectedElement:function () {
      ...
      var node = CKEDITOR.tools.tryThese(// Is it native IE control type selection?
        function () {
          return self.getNative().createRange().item(0);
        }, // If a table or list is fully selected.
        function () {
          var root, retval, range = self.getRanges()[ 0 ], ancestor = range.getCommonAncestor(1,
            1), tags = { table:1, ul:1, ol:1, dl:1 };
          ...
          if (root) {
            ...
            var enlargeables = CKEDITOR.tools.extend(tags, CKEDITOR.dtd.$listItem,
              CKEDITOR.dtd.$tableContent), walker = new CKEDITOR.dom.walker(testRange), // Check the range is at the inner boundary of the structural element.
    ...
    getSelectedText:function () {
      ...
      var text = '', nativeSel = this.getNative();
    ...
    unlock:function (restore) {
      var doc = this.document, lockedSelection = doc.getCustomData('cke_locked_selection');
...
( function () {
  var notWhitespaces = CKEDITOR.dom.walker.whitespaces(true), fillerTextRegex = /\ufeff|\u00a0/, nonCells = { table:1, tbody:1, tr:1 };

  CKEDITOR.dom.range.prototype.select = CKEDITOR.env.ie ? // V2
    function (forceExpand) {
      var collapsed = this.collapsed, isStartMarkerAlone, dummySpan, ieRange;
      ...
      if (endNode) {
        ...
        isStartMarkerAlone = ( !( next && next.getText && next.getText().match(fillerTextRegex) )     // already a filler there? && ( forceExpand || !startNode.hasPrevious() || ( startNode.getPrevious().is && startNode.getPrevious().is('br') ) ) );
