...
(function () {
  ...
  function divDialog(editor, command) {
    ...
    function createDiv(editor) {
      ...
      var selection = editor.document.getSelection(), ranges = selection.getRanges();
    ...
    function getDiv(editor) {
      var path = new CKEDITOR.dom.elementPath(editor.getSelection().getStartElement()), blockLimit = path.blockLimit, div = blockLimit && blockLimit.getAscendant('div',
        true);
      return div;
    }
    ...
    function groupByDivLimit(nodes) {
      var groups = [], lastDivLimit = null, path, block;
    ...
    function commitInternally(targetFields) {
      var dialog = this.getDialog(), element = dialog._element && dialog._element.clone() || new CKEDITOR.dom.element('div',
        editor.document);
