...
(function () {
  var commandName = 'scaytcheck', openPage = '';
  ...
  var onEngineLoad = function () {
    ...
    editor.on('destroy', function (ev) {
      var editor = ev.editor, scayt_instance = plugin.getScayt(editor);
  ...
  CKEDITOR.plugins.scayt = {
    ...
    getUiTabs:function (editor) {
      ...
      for (var i = 0; i < 4; i++) {
        uiTabs[i] = (typeof window.scayt != "undefined" && typeof window.scayt.uiTags != "undefined") ? (parseInt(configUiTabs[i],
          10) && window.scayt.uiTags[i]) : parseInt(configUiTabs[i], 10);
      }
      return uiTabs;
    },
    loadEngine:function (editor) {
      ...
      if (this.engineLoaded === true) {
        ...
        if (this.engineLoaded == -1)      // We are waiting.
        {
  ...
  var commandDefinition = {
    ...
    exec:function (editor) {
      if (plugin.isScaytReady(editor)) {
        ...
        if (!editor.config.scayt_autoStartup && plugin.engineLoaded >= 0)  // Load first time
        {
  ...
  CKEDITOR.plugins.add('scayt', {
      ...
      beforeInit:function (editor) {
        var items_order = editor.config.scayt_contextMenuItemsOrder || 'suggest|moresuggest|control', items_order_str = "";
      ...
      init:function (editor) {
        ...
        if (editor.contextMenu && editor.addMenuItems) {
          editor.contextMenu.addListener(function (element, selection) {
            ...
            var sLang = scayt_control.getLang(), _r = {}, items_suggestion = window.scayt.getSuggestion(word, sLang);
            ...
            for (var i = 0, l = items_suggestion.length; i < l; i += 1) {
              ...
              if (i < maxSuggestions) {
                addButtonCommand(editor, 'button_' + commandName, items_suggestion[i], commandName, exec, 'scayt_suggest',
                  i + 1);
                ...
                if (moreSuggestionsUnable == 'on') {
                  addButtonCommand(editor, 'button_' + commandName, items_suggestion[i], commandName, exec,
                    'scayt_moresuggest', i + 1);
            ...
            if (in_array('all', contextCommands) || in_array('ignoreall', contextCommands)) {
              ...
              addButtonCommand(editor, 'ignore_all', lang.ignoreAll, 'scayt_ignore_all', ignore_all_command, 'scayt_control',
                2);
      ...
      afterInit:function (editor) {
        ...
        var elementsPathFilters, scaytFilter = function (element) {
