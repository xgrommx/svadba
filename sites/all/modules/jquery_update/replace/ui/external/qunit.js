        ...
        if (tests) {
          ...
          addEvent(b, "dblclick", function (e) {
            ...
            if (window.location && target.nodeName.toLowerCase() === "strong") {
              window.location.search = "?" + encodeURIComponent(getText([target]).replace(/\(.+\)$/,
                "").replace(/(^\s*|\s*$)/g, ""));
            }
          });
  ...
  extend(QUnit, {
    ...
    triggerEvent:function (elem, type, event) {
      if (document.createEvent) {
        ...
        event.initMouseEvent(type, true, true, elem.ownerDocument.defaultView, 0, 0, 0, 0, 0, false, false, false, false, 0,
          null);
    ...
    var jsDump = {
            ...
            if (QUnit.is("RegExp", obj)) {
              ...
              if (QUnit.is("Date", obj)) {
                ...
                if (QUnit.is("Function", obj)) {
                  ...
                  if (obj.setInterval && obj.document && !obj.nodeType) {
                    type = "window";
                  }
                  else {
                    if (obj.nodeType === 9) {
                      ...
                      if (obj.nodeType) {
                        ...
                        if (typeof obj === "object" && typeof obj.length === "number" && obj.length >= 0) {
                          type = "array";
                        }
                        else {
                          type = typeof obj;
      ...
      parsers:{
        ...
        'function':function (fn) {
          var ret = 'function', name = 'name' in fn ? fn.name : (reName.exec(fn) || [])[1];//functions never have name in IE
          if (name) {
            ret += ' ' + name;
          }
          ret += '(';
