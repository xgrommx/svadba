...
class panels_renderer_standard {
  ...
  function init($plugin, &$display) {
    ...
    if (empty($this->plugins['layout'])) {
      watchdog('panels', "Layout: @layout couldn't been found, maybe the theme is disabled.",
  ...
  function render_layout() {
    ...
    $this->rendered['layout'] = theme($theme, array(
      'css_id' => check_plain($this->display->css_id), 'content' => $this->rendered['regions'],
      'settings' => $this->display->layout_settings, 'display' => $this->display, 'layout' => $this->plugins['layout'],
      'renderer' => $this
    ));
...
//        }
      ...
      if (!empty($pane->style['style'])) {
        ...
        if (isset($style) && isset($style['render pane'])) {
          $output = theme($style['render pane'], array(
            'content' => $content, 'pane' => $pane, 'display' => $this->display, 'style' => $style,
  ...
  function render_pane_content(&$pane) {
    ...
    if (!is_array($this->display->context)) {
      watchdog('panels', 'renderer::render_pane_content() hit with a non-array for the context', $this->display,
        WATCHDOG_DEBUG);
    ...
    if ($caching && ($cache = panels_get_cached_content($this->display, $this->display->args, $this->display->context,
      $pane))
    ) {
      ...
      $content = ctools_content_render($pane->type, $pane->subtype, $pane->configuration, array(), $this->display->args,
  ...
  function render_region($region_id, $panes) {
    ...
    return theme($style['render region'], array(
      'display' => $this->display, 'owner_id' => $owner_id, 'panes' => $panes, 'settings' => $style_settings,
      'region_id' => $region_id, 'style' => $style
    ));
  }
}
