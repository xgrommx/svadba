...
class panels_renderer_editor extends panels_renderer_standard {
  ...
  function get_display_links() {
    ...
    if (user_access('administer panels styles')) {
      ...
      $links[] = array(
        'title' => '<span class="dropdown-header">' . t('Style') . '</span>' . theme_links(array(
          'links' => $style_links, 'attributes' => array(), 'heading' => array()
        )),
        'html' => TRUE,
    ...
    if (user_access('use panels caching features')) {
      ...
      $links[] = array(
        'title' => '<span class="dropdown-header">' . t('Caching') . '</span>' . theme_links(array(
          'links' => $cache_links, 'attributes' => array(), 'heading' => array()
        )),
        'html' => TRUE,
        'attributes' => array('class' => array('panels-sub-menu')),
      );
    ...
    return theme('ctools_dropdown',
      array('title' => t('Display settings'), 'links' => $links, 'class' => 'panels-display-links'));
  }
  ...
  function get_region_links($region_id) {
    ...
    return theme('ctools_dropdown', array(
      'title' => theme('image', array('path' => ctools_image_path('icon-addcontent.png', 'panels'))), 'links' => $links,
      'image' => TRUE, 'class' => 'pane-add-link panels-region-links-' . $region_id
    ));
  }
  ...
  function get_pane_links($pane, $content_type) {
    ...
    if (user_access('administer pane access')) {
      ...
      if (!empty($pane->access['plugins'])) {
        foreach ($pane->access['plugins'] as $id => $test) {
          ...
          $access_title = isset($plugin['title']) ? $plugin['title'] : t('Broken/missing access plugin %plugin',
            array('%plugin' => $test['name']));
      ...
      $links[] = array(
        'title' => '<span class="dropdown-header">' . t('Visibility rules') . '</span>' . theme_links(array(
          'links' => $visibility_links, 'attributes' => array(), 'heading' => array()
        )),
        'html' => TRUE,
        'attributes' => array('class' => array('panels-sub-menu')),
      );
    ...
    if (user_access('use panels locks')) {
      ...
      $links[] = array(
        'title' => '<span class="dropdown-header">' . t('Locking') . '</span>' . theme_links(array(
          'links' => $lock_links, 'attributes' => array(), 'heading' => array()
        )),
        'html' => TRUE,
        'attributes' => array('class' => array('panels-sub-menu')),
      );
    ...
    return theme('ctools_dropdown', array(
      'title' => theme('image', array('path' => ctools_image_path('icon-configure.png', 'panels'))), 'links' => $links,
      'image' => TRUE
    ));
  }
  ...
  function ajax_add_pane($region = NULL, $type_name = NULL, $subtype_name = NULL, $step = NULL) {
    ...
    $output = ctools_content_form('add', $form_info, $form_state, $content_type, $pane->subtype, $subtype,
      $pane->configuration, $step);
  ...
  function ajax_edit_pane($pid = NULL, $step = NULL) {
    ...
    $output = ctools_content_form('edit', $form_info, $form_state, $content_type, $pane->subtype, $subtype,
      $pane->configuration, $step);
  ...
  function ajax_style_settings($type, $pid = '') {
    ...
    switch ($type) {
      ...
      case 'region':
        $title = t('Style settings for style @style (Region "!region")',
          array('@style' => $style['title'], '!region' => $this->plugins['layout']['regions'][$pid]));
        break;
      ...
      case 'pane':
        ...
        $pane = &$this->display->content[$pid];
        $subtype = ctools_content_get_subtype($pane->type, $pane->subtype);
        $title = t('Style settings for style @style (Pane "!pane")',
          array('@style' => $style['title'], '!pane' => $subtype['title']));
        break;
    }
