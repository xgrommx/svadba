(function ($) {
  ...
  Drupal.CTools.Stylizer.addFarbtastic = function (context) {
    ...
    function focus() {
      var input = this;
      // Remove old bindings
      focused && $(focused).unbind('keyup', farb.updateValue).unbind('keyup',
        resetScheme).parent().removeClass('item-selected');
      ...
      $(focused).keyup(farb.updateValue).keyup(resetScheme).parent().addClass('item-selected');
    }
    ...
    $('#palette input.form-text', form).each(function () {
        ...
        var i = inputs.length;
        if (inputs.length) {
          var lock = $('<div class="lock"></div>').toggle(function () {
              $(this).addClass('unlocked');
              $(hooks[i - 1]).attr('class', locks[i - 2] && $(locks[i - 2]).is(':not(.unlocked)') ? 'hook up' : 'hook');
              $(hooks[i]).attr('class', locks[i] && $(locks[i]).is(':not(.unlocked)') ? 'hook down' : 'hook');
            }, function () {
              $(this).removeClass('unlocked');
              $(hooks[i - 1]).attr('class',
                locks[i - 2] && $(locks[i - 2]).is(':not(.unlocked)') ? 'hook both' : 'hook down');
              $(hooks[i]).attr('class', locks[i] && $(locks[i]).is(':not(.unlocked)') ? 'hook both' : 'hook up');
            });
          $(this).after(lock);
          locks.push(lock);
        }
  ...
  Drupal.behaviors.CToolsColorSettings = {
    attach:function () {
      $('.ctools-stylizer-color-edit:not(.ctools-color-processed)').addClass('ctools-color-processed').each(function () {
          Drupal.CTools.Stylizer.addFarbtastic('#' + $(this).attr('id'));
        });

      $('div.form-item div.ctools-style-icon:not(.ctools-color-processed)').addClass('ctools-color-processed').click(function () {
