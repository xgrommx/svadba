...
class ctools_export_ui {
  ...
  function access($op, $item) {
    ...
    if (!empty($this->plugin['allowed operations'][$op]['token']) && (!isset($_GET['token']) || !drupal_valid_token($_GET['token'],
      $op))
    ) {
      return FALSE;
    }
  ...
  function list_form(&$form, &$form_state) {
    ...
    $form['#attached']['js'] = array(
      'misc/ajax.js', 'misc/progress.js', 'misc/jquery.form.js', ctools_attach_js('auto-submit')
    );
  ...
  function list_build_row($item, &$form_state, $operations) {
    ...
    if (!empty($this->plugin['export']['admin_title'])) {
      $this->rows[$name]['data'][] = array(
        'data' => check_plain($item->{$this->plugin['export']['admin_title']}), 'class' => array('ctools-export-ui-title')
      );
    }
    $this->rows[$name]['data'][] = array('data' => check_plain($name), 'class' => array('ctools-export-ui-name'));
    $this->rows[$name]['data'][] = array(
      'data' => check_plain($item->{$schema['export']['export type string']}), 'class' => array('ctools-export-ui-storage')
    );
  ...
  function edit_execute_form_wizard(&$form_state) {
    ...
    if (!empty($form_state['complete'])) {
      ...
      if ($output && !empty($form_state['item']->export_ui_item_is_cached)) {
        ...
        drupal_set_message(t('You have unsaved changes. These changes will not be made permanent until you click <em>Save</em>.'),
          'warning');
      }
    }
  ...
  function edit_save_form($form_state) {
    ...
    if ($result) {
      $message = str_replace('%title', check_plain($item->{$export_key}),
        $this->plugin['strings']['confirmation'][$form_state['op']]['success']);
      drupal_set_message($message);
    }
    else {
      $message = str_replace('%title', check_plain($item->{$export_key}),
        $this->plugin['strings']['confirmation'][$form_state['op']]['fail']);
  ...
  function edit_form_import_validate($form, &$form_state) {
    ...
    if (is_string($item)) {
      form_error($form['import'],
        t('Unable to get an import from the code. Errors reported: @errors', array('@errors' => $item)));
      return;
    }
...
function ctools_export_ui_edit_item_form_delete(&$form, &$form_state) {
  ...
  drupal_goto(ctools_export_ui_plugin_menu_path($form_state['plugin'], $path, $form_state['item']->{$export_key}),
    array('cancel_path' => $_GET['q']));
}
...
function ctools_export_ui_edit_name_validate($element, &$form_state) {
  ...
  if (empty($form_state['item']->export_ui_allow_overwrite) && $exists = ctools_export_crud_load($plugin['schema'],
    $element['#value'])
  ) {
    form_error($element,
      t('A @plugin with this name already exists. Please choose another name or delete the existing item before creating a new one.',
        array('@plugin' => $plugin['title singular'])));
  }
}
...
function ctools_export_ui_delete_confirm_form($form, &$form_state) {
  ...
  $question = str_replace('%title', check_plain($item->{$export_key}),
    $plugin['strings']['confirmation'][$form_state['op']]['question']);
